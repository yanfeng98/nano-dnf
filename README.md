...
```bash
npm test          # 136 个核心逻辑、Boss/小 Boss 机制、强化与通关记录、配乐调度、标题演示、通关战绩、发布暂存契约与渲染冒烟测试
...
通关画面本身也给出这一局的战绩：`src/summary.js`（纯函数、无 DOM）把
**种子 / 用时 / 等级 / 强化 / 击杀 / 承伤**排成两列，画在 `DUNGEON CLEARED` 下方。
这些数字来自通关那一刻的快照——`state.time` 在结束画面背后仍然在走，所以直接读实时 state
会让表里的用时和上面「本次」的时间对不上；主循环在记账那一帧就把这一局冻结下来。
...
| `src/summary.js` | 通关战绩格式化：种子 / 用时 / 等级 / 强化 / 击杀 / 承伤（纯函数，可单测） |
...
| `tests/summary.test.js` | 验证通关战绩：逐格数值、强化名映射、异常输入不吐脏数字、时间格式与 `records.js` 完全一致 |
