...
```bash
npm test          # 137 个核心逻辑、Boss/小 Boss 机制、强化与通关记录、配乐调度、标题演示、通关/失败战绩、发布暂存契约与渲染冒烟测试
...
| `src/summary.js` | 结算战绩格式化：种子 / 用时 / 等级 / 强化 / 击杀 / 承伤 / 到达层数（纯函数，通关与失败共用） |
...
| `tests/summary.test.js` | 验证结算战绩：逐格数值、到达层数夹取、强化名映射、异常输入不吐脏数字、时间格式与 `records.js` 完全一致 |
